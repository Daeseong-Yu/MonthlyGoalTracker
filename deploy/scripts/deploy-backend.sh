...
load_api_env() {
  ...
  unset APP_LOGIN_RATE_LIMIT_PER_MINUTE APP_LEGACY_CLAIM_TOKEN
  ...
  while IFS= read -r line || [ -n "${line}" ]; do
    ...
    case "${key}" in
      APP_HOST|APP_PORT|DATABASE_URL|APP_BASIC_AUTH_USERNAME|APP_BASIC_AUTH_PASSWORD_HASH|APP_SESSION_COOKIE_NAME|APP_CSRF_COOKIE_NAME|APP_SESSION_TTL_HOURS|APP_COOKIE_SECURE|APP_COOKIE_SAMESITE|APP_SIGNUP_RATE_LIMIT_PER_MINUTE|APP_LOGIN_RATE_LIMIT_PER_MINUTE|APP_LEGACY_CLAIM_TOKEN)
        export "${key}=${value}"
        ;;
    esac
  done < "${api_env_file}"

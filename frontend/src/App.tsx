  ...
  return (
    <main className="min-h-screen bg-[#f7f8f5] text-zinc-900">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8">
        ...
        <MetricSummary
          activeGoalCount={referencePoint?.activeGoalCount ?? 0}
        ...
        <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_20.5rem]">
          <DailyRecordTable
            ...
            chartData={chartData}
            checks={checks}
            dailyRecordGoalSlots={dailyRecordGoalSlots}
            days={days}
            isMutatingMonth={isMutatingMonth}
            savingChecks={savingChecks}
            savingMemos={savingMemos}
            onMemoBlur={(date, memo) => void saveMemoForDate(date, memo)}
            onMemoChange={updateMemo}
            onToggleCheck={(goalId, date) => void toggleCheck(goalId, date)}
          />

          <aside className="space-y-6">
            <GoalPanel
              canSaveChanges={canSaveChanges}
              deactivatingGoalIDs={deactivatingGoalIDs}
              editingGoalID={editingGoalID}
              editingGoalTitle={editingGoalTitle}
              goalFormOpen={goalFormOpen}
              goalListReferenceDate={goalListReferenceDate}
              isMutatingMonth={isMutatingMonth}
              month={month}
              newGoalStartDate={newGoalStartDate}
              newGoalTitle={newGoalTitle}
              visibleGoals={visibleGoals}
              onCancelEditingGoal={cancelEditingGoal}
              onDeactivateGoal={(goal) => void deactivateGoalFromMonth(goal)}
              onEditingGoalTitleChange={setEditingGoalTitle}
              onNewGoalStartDateChange={setNewGoalStartDate}
              onNewGoalTitleChange={setNewGoalTitle}
              onStartEditingGoal={startEditingGoal}
              onSubmitGoalTitle={(event, goalID) =>
                void submitGoalTitle(event, goalID)
              }
              onSubmitNewGoal={(event) => void submitNewGoal(event)}
              onToggleGoalForm={() => {
                cancelEditingGoal();
